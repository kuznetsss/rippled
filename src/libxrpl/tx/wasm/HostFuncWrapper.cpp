...
checkGas(WasmRuntimeWrapper& rt, int64_t delta)
{
    ...
    if (rt.setGas(x) < 0)
        return std::unexpected(HostFunctionError::Internal);  // LCOV_EXCL_LINE

    if (gas < delta)
        return std::unexpected(HostFunctionError::OutOfGas);

    return x;
}

static inline std::expected<std::int64_t, wasm_trap_t*>
checkGas(WasmRuntimeWrapper& rt, WasmImportFunc const& impFunc)
{
    ...
    if (!g)
    {
        ...
        return std::unexpected(trap);                          // LCOV_EXCL_LINE
    }
...
static std::expected<std::tuple<HostFunctions&, WasmImportFunc const&>, wasm_trap_t*>
mainCheck(void* env, wasm_val_vec_t const* params, wasm_val_vec_t* results)
{
    ...
    if (auto g = checkGas(rt, impFunc); !g)
        return std::unexpected(g.error());  // LCOV_EXCL_LINE
